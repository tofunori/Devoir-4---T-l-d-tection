...
Le satellite Landsat 8, opéré par la NASA et l'USGS \citep{USGS2022}, embarque un capteur multispectral qui offre une résolution spatiale de 30 mètres pour les bandes visibles et proche infrarouge (NIR), 15 mètres pour la bande panchromatique, et 100 mètres pour les bandes thermiques (TIRS). Sa résolution spectrale est remarquable avec 11 bandes couvrant un spectre électromagnétique allant du visible à l'infrarouge thermique. Le satellite Landsat 8 suit une orbite héliosynchrone à 705 km d'altitude avec une inclinaison de 98,2 degrés, ce qui permet une couverture complète de la Terre tous les 16 jours (résolution temporelle). La technologie d'acquisition "pushbroom" employée par Landsat 8, qui représente une évolution par rapport aux scanners à miroir tournant des générations précédentes, permet de capturer des images sur une fauchée de 185 km de large en un seul passage, offrant ainsi une vue complète du phénomène volcanique.

L'image du volcan est présentée en composition fausses couleurs 6-5-3 (SWIR-NIR-Vert), afin de mettre en évidence l'activité volcanique. Les coulées de lave récentes apparaissent en rouge/orange vif, conséquence directe de leur forte émission thermique captée dans la bande 6 (SWIR, 1.57-1.65 $\mu$m). Cette observation s'appuie sur la loi de Planck qui régit l'émission électromagnétique des corps chauds, dont le pic d'émission se déplace vers les longueurs d'onde plus courtes à mesure que la température augmente. Les coulées de lave atteignent généralement des températures entre 700$^\circ$C et 1200$^\circ$C, ce qui génère une signature thermique intense dans cette gamme spectrale du SWIR. À ces températures, l'émission radiative devient suffisamment forte pour être détectée même à travers les nuages ou la cendre des volcans.

La végétation de l'île se distingue par sa teinte vert vif,  résultat de sa haute réflectance dans la bande 5 (NIR, 0.85-0.88 \textmu m). Cette forte réflexion dans le proche infrarouge vient de la structure interne des feuilles qui diffuse la lumière à l'intérieur des tissus végétaux. Le cratère central du volcan apparaît en bleu foncé/noir, ce qui montre l'absence de végétation et une faible réflectance dans les trois bandes spectrales utilisées. Les zones blanches visibles correspondent à des nuages ou à des émissions de vapeur et de cendres qui réfléchissent fortement le rayonnement dans l'ensemble des bandes spectrales utilisées.
...
\begin{itemize}
    ...
    \url{https://code.earthengine.google.com/bca7396ed58795881bbbed32195a3344}
    
    \item Code pour l'analyse de l'éruption volcanique La Cumbre (Landsat 8): 
    \url{https://code.earthengine.google.com/5c8de3b163ca4f57866f97eb99038238}
\end{itemize}
\end{document}